...
type BlobTests() =
    ...
    [<Test>]
    member x.TestCreateIfMissingAsync() =
        async {
            let name = name()
            let data = bytes()

            let! uri = Store.Blob.createIfMissingAsync container name (async { return data })
            match uri with
            | None ->
                if container.GetBlobReference(name).Exists() then
                    Assert.Fail("Should have returned an URI for the initial blob that was created.")
            ...
            | Some uri ->
                ...
                let! uri = Store.Blob.createIfMissingAsync container name (async { return bytes() })
                let contents =
                    use stream = new MemoryStream()
                    blob.DownloadToStream(stream)
                    stream.ToArray()
    ...
    [<Test>]
    member x.TestUpdateBytesAsync() =
        async {
            let name = name()
            let updateAsync = Store.Blob.updateAsync Store.Retry.immediately container (id,id) name
            let incrementAsync : Store.Blob.UpdateAsync<byte[]> =
                fun input -> async {
                    return
                        match input with
                        | None -> "1"
            ...
            for i in 1..10 do
                let! result = updateAsync incrementAsync
                results.Add(result)
            let results =
                results
                |> List.ofSeq
